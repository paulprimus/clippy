...
class Change:
    ...
    def __post_init__(self) -> None:
        if self.record_type not in ("1", "2", "u", "?", "!"):
            raise ValueError(
                f"Nicht unterstützter Porcelain-v2-Eintragstyp: {self.record_type!r}"
            )
        if not self.path:
        ...
        if self.record_type in ("?", "!"):
            if self.xy is not None or self.sub is not None:
                raise ValueError(
                    "Nicht versionierte und ignorierte Einträge besitzen weder ein xy- noch ein sub-Feld."
                )
            if self.modes or self.object_ids or self.original_path or self.score:
                raise ValueError(
                    "Nicht versionierte und ignorierte Einträge besitzen keine weiteren Felder."
                )
            return
        ...
        if (
            self.xy is None
            or len(self.xy) != 2
            or any(status not in ".MTADRCU" for status in self.xy)
        ):
            raise ValueError(
                "xy muss aus zwei gültigen Statuszeichen des Porcelain-v2-Formats bestehen."
            )
        if self.sub is None or not (
            ...
            raise ValueError(
                "sub muss 'N...' oder einen gültigen Submodulstatus enthalten."
            )

        if self.record_type in ("1", "2"):
            if len(self.modes) != 3 or len(self.object_ids) != 2:
                raise ValueError(
                    "Normale und umbenannte Einträge benötigen 3 Modi und 2 Objekt-IDs."
                )
        ...
        if self.record_type == "2":
            if not self.original_path or self.score is None:
                raise ValueError(
                    "Umbenannte oder kopierte Einträge benötigen den ursprünglichen Pfad und einen Ähnlichkeitswert."
                )
            if (
                len(self.score) < 2
                or self.score[0] not in "RC"
                or not self.score[1:].isdigit()
            ):
                raise ValueError(
                    "Der Ähnlichkeitswert muss ein Umbenennungs- oder Kopierwert sein, zum Beispiel 'R100'."
                )
            ...
            raise ValueError(
                "Nur umbenannte oder kopierte Einträge besitzen einen ursprünglichen Pfad und einen Ähnlichkeitswert."
            )
